...
//    warsawStopDirectionService.isDirection(LINE, DIRECTION)
    ...
    def "shouldReturnMaximumAndMinimumDeparture"() {
        given: "mock stops in Warsaw"
        warsawApiClient.getStops(API_KEY, STOP_RESOURCE_ID) >> ok(readJson("/warsaw/stops-narutowicza.json", pl.wrona.warsaw.transport.api.model.WarsawStops.class))

        and: "mock timetables"
        warsawApiClient.getTimetable(API_KEY, TIMETABLE_RESOURCE_ID, "4121", "03", "9") >> ok(readJson("/warsaw/timetable-4121-03-9.json", WarsawTimetables.class))

        when: "get vehicle departure on stop in Warsaw"
        WarsawDepartures morningEdgeTimetable = warsawTimetableService.getMorningEdgeTimetable()
        WarsawDepartures eveningEdgeTimetable = warsawTimetableService.getEveningEdgeTimetable()

        then:
        morningEdgeTimetable.getTime().toLocalTime() == LocalTime.of(4, 55)
        eveningEdgeTimetable.getTime().toLocalTime() == LocalTime.of(0, 17)
    }
